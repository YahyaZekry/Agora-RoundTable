...
2. Wherever `<personas>/<slug>/` exists (check both locations), delete only its
   ...
   `inbox/_sync-status.md` (not the rest of `inbox/` — those are the user's dropped
   files, not cache; deleting just the manifest forces Step 5.5 to re-extract all of
   them into the freshly rebuilt `research/`). That's the entire cache for this
